...
create function F_GetRoleId(@currentUserId varchar(32))
	returns @RoldIdTable table (id varchar(32))
as
begin
	insert into @RoldIdTable select right_id from sys_user_role_links where left_id = @currentUserId;
	return;
end
go
...
create function F_GetDeptId(@currentUserId varchar(32))
	returns @DeptIdTable table (id varchar(32))
as
begin
	insert into @DeptIdTable select right_id from sys_user_dept_links where left_id = @currentUserId;
	return;
end
go
