...
public class DynamicBasicDataColumnUtil {
	...
	public static void initBasicColumnToTable(ComTabledata table){
		// id
		ComColumndata idColumn = new ComColumndata("id", BuiltinCodeDataType.STRING, 32);
		...
		// customerId
		ComColumndata customerIdColumn = new ComColumndata("customer_id", BuiltinCodeDataType.STRING, 32);
		...
		if(table.getIsDatalinkTable() == 0 && !table.getTableName().endsWith("_LINKS")){// 不是关系表，才要这些字段
			ComColumndata createTimeColumn = new ComColumndata("create_date", BuiltinCodeDataType.DATE, 0);
			...
			ComColumndata lastUpdateTimeColumn = new ComColumndata("last_update_date", BuiltinCodeDataType.DATE, 0);
			...
			ComColumndata createUserIdColumn = new ComColumndata("create_user_id", BuiltinCodeDataType.STRING, 32);
