...
public class WfReResource extends BasicEntity implements IEntity, IEntityPropAnalysis{
	...
	private String processConfigXmlContent;
	 ...
	 * @return
	 */
	public String getProcessConfigXmlContent(){
		if(isProcessConfigXmlFile() && processConfigXmlContent == null && byteContent != null && byteContent.length > 0){
			try {
				processConfigXmlContent = new String(byteContent, EncodingConstants.UTF_8);
			} catch (UnsupportedEncodingException e) {
				throw new IllegalArgumentException("在获取流程配置的xml内容时，将byte数组转换成utf8编码格式的字符串时出现异常：" + ExceptionUtil.getErrMsg(e));
			}
		}
		return processConfigXmlContent;
	}
	public void setProcessConfigXmlContent(String processConfigXmlContent) {
		if(StrUtils.notEmpty(processConfigXmlContent)){
			this.processConfigXmlContent = processConfigXmlContent;
			try {
				this.byteContent = processConfigXmlContent.getBytes(EncodingConstants.UTF_8);
			} catch (UnsupportedEncodingException e) {
				throw new IllegalArgumentException("在set流程配置的xml内容时，将字符串转换为utf8编码格式的byte数组时出现异常：" + ExceptionUtil.getErrMsg(e));
			}
		}
	}
	 ...
	 */
	@JSONField(serialize = false)
	public boolean isProcessConfigXmlFile(){
